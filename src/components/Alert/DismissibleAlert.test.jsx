...
describe('<DismissableAlert />', () => {
	...
	describe('Dismiss behavior', ()=> {
      ...
      const customDismissBehavior = jest.fn();
			render(<DismissableAlert title={titleText} variant="info" onDismiss={customDismissBehavior}/>);
			expect(screen.getByRole('button')).toBeVisible();
		});
		it('should fire dismiss delegate', async () => {
			let fired = false;
			const delegate = () => fired = true;
			render(<DismissableAlert title={titleText} variant="info" onDismiss={delegate} />);
			await user.click(screen.getByRole('button'));

			expect(fired).toEqual(true);
		});
		it('the alert should no longer remain visible when dismiss button clicked', async () => {
			render(<DismissableAlert title={titleText} variant="info" onDismiss={() => {;}} />);
