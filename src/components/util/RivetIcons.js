...
const propTypes = {
    /** The icon name */
    name: PropTypes.oneOf(["caret-down", "caret-right", "close", "file", "menu", "info", "success", "warning", "error", "trident-header", "trident-footer"])
}
...
const resolveContents = (name, attrs) => {
    switch (name) {
        ...
        case "info": return icoInfo(attrs);
        case "success": return icoSuccess(attrs);
        case "warning": return icoWarning(attrs);
...
const Icon = ({ name, ...attrs }) => resolveContents(name, attrs);
